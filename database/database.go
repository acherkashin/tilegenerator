...
func (gdb *GeometryDB) rowsToMapObjects(rows *sql.Rows) ([]entities.MapObject, error) {
	...
	for tmpRows.Next() {
		var ID, typeID int
		var wkt, label, textPosition, colorOuter, colorInner string
		...
		err := tmpRows.Scan(&ID, &typeID, &wkt, &label, &isShortwaveAntenna, &needShowAzimuthalGrid, &beamWidth, &sidelobes, &azimut, &distance, &needShowDirectionalDiagram, &textPosition, &colorOuter, &colorInner)
...
func (gdb *GeometryDB) GetGeometriesForTile(tile *tiles.Tile, situationsIds string) (mapObjects []entities.MapObject, err error) {
	...
	q := fmt.Sprintf(`
		...
		coalesce(color_outer, ''), coalesce(color_inner, '')  from %s
