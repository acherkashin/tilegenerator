...
func (gdb *GeometryDB) rowsToMapObjects(rows *sql.Rows) ([]entities.MapObject, error) {
	...
	for tmpRows.Next() {
		var ID, typeID int
		var wkt, label, textPosition string
		...
		err := tmpRows.Scan(&ID, &typeID, &wkt, &label, &isShortwaveAntenna, &needShowAzimuthalGrid, &beamWidth, &sidelobes, &azimut, &distance, &needShowDirectionalDiagram, &textPosition)

		if err == nil {
			mapObj, mapObjErr := entities.NewObject(ID, typeID, wkt, isShortwaveAntenna, needShowAzimuthalGrid, needShowDirectionalDiagram, beamWidth, sidelobes, azimut, distance)
			if mapObjErr == nil {
				mapObj.Label = label
...
func (gdb *GeometryDB) GetGeometriesForTile(tile *tiles.Tile, situationsIds string) (mapObjects []entities.MapObject, err error) {
	q := fmt.Sprintf(`
		SELECT id,type_id, ST_AsText( ST_Transform( %s, 4326 ) ), coalesce(text1, ''), coalesce(is_shortwave_antenna, false), coalesce(need_show_azimuthal_grid, false), coalesce(beam_width, '1'), coalesce(sidelobes, '1'), coalesce(azimut, '0'),  coalesce(distance, '0'), coalesce(need_show_directional_diagram, 'false'), coalesce(text_position, 'bottom') from %s
		WHERE type_id NOT in (170, 11) and 
		(min_zoom <= %v or min_zoom is null) and
		(max_zoom >= %v or max_zoom is null) and
		situation_id in (%v) and
		ST_Intersects(ST_SetSRID(ST_MakeBox2D(ST_Point(%v, %v), ST_Point(%v, %v)), 4326), the_geom);
		`, gdb.geomcol, gdb.geomtable, tile.Z, tile.Z, situationsIds, tile.BoundingBox.West, tile.BoundingBox.North, tile.BoundingBox.East, tile.BoundingBox.South)
...
func (gdb *GeometryDB) GetAllSpecialObject(tile *tiles.Tile, situationsIds string) (mapObjects []entities.MapObject, err error) {
	q := fmt.Sprintf(`SELECT id,type_id, ST_AsText( ST_Transform( %s, 4326 ) ), coalesce(text1, ''), coalesce(is_shortwave_antenna, false),
		...
		(max_zoom >= %v or max_zoom is null) and
		situation_id in (%v) and
		ST_Intersects(ST_SetSRID(ST_MakeBox2D(ST_Point(%v, %v), ST_Point(%v, %v)), 4326), the_geom);`, gdb.geomcol, gdb.geomtable, tile.Z, tile.Z, situationsIds, tile.BoundingBox.West, tile.BoundingBox.North, tile.BoundingBox.East, tile.BoundingBox.South)
