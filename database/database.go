...
func (gdb *GeometryDB) rowsToMapObjects(rows *sql.Rows) ([]entities.MapObject, error) {
	...
	for tmpRows.Next() {
		...
		var label, colorOuter, colorInner string
		...
		err := tmpRows.Scan(&ID, &typeID, &wkt, &label, &isShortwaveAntenna, &needShowAzimuthalGrid, &beamWidth, &sidelobes, &azimut, &distance, &needShowDirectionalDiagram, &colorOuter, &colorInner)

		if err == nil {
			mapObj, mapObjErr := entities.NewObject(ID, typeID, wkt, isShortwaveAntenna, needShowAzimuthalGrid, needShowDirectionalDiagram, beamWidth, sidelobes, azimut, distance, colorOuter, colorInner)
			if mapObjErr == nil {
				mapObj.Label = label
...
func (gdb *GeometryDB) GetGeometriesForTile(tile *tiles.Tile) (mapObjects []entities.MapObject, err error) {
	q := fmt.Sprintf(`
		...
		`, gdb.geomcol, gdb.geomtable, tile.Z, tile.Z, tile.BoundingBox.West, tile.BoundingBox.North, tile.BoundingBox.East, tile.BoundingBox.South)
